...
type InputType = 'date' | 'email' | 'number' | 'password' | 'search' | 'tel' | 'text' | 'time' | 'url' | 'datetime-local' | 'week'
...
export const propKeys = ['id', 'ids', 'required', 'disabled', 'invalid', 'readOnly', 'type'] as const

export const useFieldService = (props: UseFieldServiceProps) => {
  const { ids, disabled = false, invalid = false, readOnly = false, required = false, type = 'text' } = props
  ...
  const getRootProps = useMemo(
    ...
    [rootRef, disabled, invalid, readOnly, rootId],
  )
  ...
  return {
    isPasswordVisible,
    togglePasswordVisibility: () => setPasswordInputType(prev => (prev === 'text' ? 'password' : 'text')),
    getRootProps,
    getLabelProps,
    getErrorTextProps,
